...
public class JwtServiceImpl implements JwtService {
    ...
    public String generateToken(UserDetails userDetails) {
        //Mostrar mensaje por consola del Token generado
        System.out.println("Token generado: "+generateToken(new HashMap<>(), userDetails));
    ...
    private Key getSigningKey() {
       ...
       System.out.println("Token.Signing.Key: "+jwtSigningKey);
    ...
    private String generateToken(Map<String, Object> extraClaims, UserDetails userDetails) {
        return Jwts.builder().setClaims(extraClaims).setSubject(userDetails.getUsername()).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24)) //Pasadas 24 horas el token expira
